...
(function(global, $) {
	...
	codiad.CodeGit = {
		...
		init: function() {
			...
			amplify.subscribe('context-menu.onShow', function(obj) {
				var path = $(obj.e.target).attr('data-path'),
					...
					counter = 0;
				if ($(obj.e.target).data('type') == 'directory') {
				...
				$.getJSON(_this.path + 'controller.php?action=clone&path=' + path + '&repo=' + repo + '&init_submodules=' + init_submodules, function(result) {
					if (result.status == 'login_required') {
						...
						_this.login = function() {
							...
							codiad.modal.unload();
							$.post(_this.path + 'controller.php?action=clone&path=' + path + '&repo=' + repo + '&init_submodules=' + init_submodules, {
									username: username,
									password: password
								},
								function(result) {
		...
		commit: function(path, msg) {
			...
			$.post(this.path + 'controller.php?action=add&path=' + path, {
				files: JSON.stringify(_this.files)
			}, function(result) {
				result = JSON.parse(result);
		...
		rename: function(fPath) {
			...
			codiad.modal.load(250, codiad.filemanager.dialog, {
				action: 'rename',
				path: fPath,
		...
		isEnabledRepoStatus: function() {
			var setting = localStorage.getItem('codiad.plugin.codegit.disableRepoStatus'),
				ret = true;
			if (setting === "true") {
				ret = false;
			}
		...
		isEnabledWrapper: function() {
			var setting = localStorage.getItem('codiad.plugin.codegit.disableHeader'),
				ret = true;
			if (setting === "true") {
				ret = false;
			}
